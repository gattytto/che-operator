...
func GetSpecKeycloakDeployment(
	...
	command := bashFunctions + "\n" + addCertToTrustStoreCommand + addProxyCliCommand + applyProxyCliCommand + " && " + changeConfigCommand + enableFixedHostNameProvider +
		" && if [ -n $(echo \"$(cat /proc/net/if_inet6 | awk '$6 != \"lo\" {print $6}')\" |head -1) ]; then BIND=\"[::1]\" /opt/jboss/docker-entrypoint.sh --debug -b [::1] -c standalone.xml -Djava.net.preferIPv4Stack=false -Dkeycloak.profile.feature.token_exchange=enabled -Dkeycloak.profile.feature.admin_fine_grained_authz=enabled -Djava.net.preferIPv6Addresses=true -Djboss.bind.address.management=\"[::1]\" -Djboss.bind.address.unsecure=\"[::1]\" && sed -i \"s/127\\.0\\.0\\.1/[\\:\\:1]/g\" /opt/jboss/keycloak/standalone/configuration/standalone.xml ; else /opt/jboss/docker-entrypoint.sh --debug -b 0.0.0.0 -c standalone.xml -Dkeycloak.profile.feature.token_exchange=enabled -Dkeycloak.profile.feature.admin_fine_grained_authz=enabled; fi;"
